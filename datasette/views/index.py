...
class IndexView(BaseView):
    ...
    async def get(self, request):
        ...
        visible = await self.ds.visible_databases_and_tables(
            request.actor, include_private=True
        )
        ...
        for db_info in visible:
            ...
            database_private = db_info["private"]

            # Tables (not views) and views from the shared visible set
            table_names = [t["name"] for t in db_info["tables"]]
            table_private = {t["name"]: t["private"] for t in db_info["tables"]}
            views = db_info["views"]
            ...
            for table in table_names:
                # Defensive: only include tables in the allowed set
                if table not in table_private:
                    continue
